...
console.log('🔧 TherapEase Middleware Undefined Fix');
console.log('=====================================\n');

// 1. Check middleware exports
console.log('1. 🔍 Checking middleware exports...');
  ...
  if (file.endsWith('.js')) {
    ...
    try {
      ...
      if (content.includes('module.exports =')) {
        ...
        console.log('     ❌ No clear export found');
      }
      
      // Check if it's a function
      if (content.includes('function') || content.includes('=>') || content.includes('(req, res, next)')) {
        console.log('     ✅ Appears to be a function');
      } else {
        console.log('     ⚠️  May not be a function');
      }
    } catch (error) {
...
// 2. Create a safe server file that checks middleware before using
console.log('\n2. 🔨 Creating safe server file...');

const safeServerContent = `const express = require('express');
...
// 3. Write the safe server file
console.log('\n3. 💾 Writing safe server file...');
const serverFile = path.join(__dirname, 'server', 'index.js');
fs.writeFileSync(serverFile, safeServerContent);
console.log('   ✅ Safe server file written');

// 4. Test syntax
console.log('\n4. ✅ Testing syntax...');
try {
  new Function(safeServerContent);
...
exec('cd server && timeout 5s node index.js', (error, stdout, stderr) => {
  ...
  exec('pm2 restart therapease-api', (error, stdout, stderr) => {
    if (error) {
      ...
      // Wait and test
      setTimeout(() => {
        console.log('\n7. 🧪 Testing server after middleware fix...');
        ...
        exec('curl -s -w "Local: %{http_code}" http://127.0.0.1:5000/health', (error, stdout, stderr) => {
          ...
          exec('curl -s -w "WebSocket: %{http_code}" https://therapease.site/ws', (error, stdout, stderr) => {
            ...
            exec('curl -s -w "API: %{http_code}" https://therapease.site/api/auth/test', (error, stdout, stderr) => {
              ...
              console.log('=========================');
              console.log('✅ Safe server file created');
              console.log('✅ Middleware safety checks added');
              console.log('✅ Fallback functions for missing middleware');
