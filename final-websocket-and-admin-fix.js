...
console.log('🎯 TherapEase Final WebSocket & Admin Fix');
console.log('=========================================\n');

// 1. Read current server file
console.log('1. 📄 Reading current server file...');
...
// The issue is that the static file serving middleware is catching /ws requests
// We need to move the WebSocket route to the very beginning and fix the static serving

const fixedServerContent = `const express = require('express');
...
// 3. Write the fixed server file
console.log('\n3. 💾 Writing fixed server file...');
fs.writeFileSync(serverFile, fixedServerContent);
console.log('   ✅ Fixed server file written');

// 4. Test syntax
console.log('\n4. ✅ Testing syntax...');
try {
  new Function(fixedServerContent);
...
exec('cd server && timeout 5s node index.js', (error, stdout, stderr) => {
  ...
  exec('pm2 restart therapease-api', (error, stdout, stderr) => {
    if (error) {
      ...
      // Wait and test
      setTimeout(() => {
        console.log('\n7. 🧪 Testing server after final fix...');
