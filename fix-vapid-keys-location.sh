...
echo "🔧 Fix VAPID Keys Location Issue"
echo "==============================="
...
echo ""
echo "🔧 Fixing VAPID Keys Location Issue"
echo "===================================="
...
echo ""
echo "🔍 Step 2: Check VAPID Keys in Server Directory"
echo "==============================================="

# Check VAPID keys in server directory
echo "Checking VAPID keys in server directory..."
if [ -f "$BASE_DIR/server/.env.production" ]; then
    ...
    grep -E "VAPID_" "$BASE_DIR/server/.env.production" || echo "No VAPID keys found in server/.env.production"
else
    print_status "WARN" ".env.production file not found in server directory"
fi

echo ""
echo "🔍 Step 3: Copy VAPID Keys from Server Directory"
echo "==============================================="

# Copy VAPID keys from server directory
echo "Copying VAPID keys from server directory..."

if [ -f "$BASE_DIR/server/.env.production" ]; then
    # Extract VAPID keys from server/.env.production
    VAPID_PUBLIC=$(grep "VAPID_PUBLIC_KEY" "$BASE_DIR/server/.env.production" | cut -d'=' -f2)
    VAPID_PRIVATE=$(grep "VAPID_PRIVATE_KEY" "$BASE_DIR/server/.env.production" | cut -d'=' -f2)
    VAPID_SUBJECT=$(grep "VAPID_SUBJECT" "$BASE_DIR/server/.env.production" | cut -d'=' -f2)
    
    if [ ! -z "$VAPID_PUBLIC" ]; then
        print_status "PASS" "VAPID public key found: ${VAPID_PUBLIC:0:20}..."
    else
        print_status "WARN" "VAPID public key not found in server/.env.production"
    fi
    
    if [ ! -z "$VAPID_PRIVATE" ]; then
        print_status "PASS" "VAPID private key found: ${VAPID_PRIVATE:0:20}..."
    else
        print_status "WARN" "VAPID private key not found in server/.env.production"
    fi
    
    if [ ! -z "$VAPID_SUBJECT" ]; then
        print_status "PASS" "VAPID subject found: $VAPID_SUBJECT"
    else
        print_status "WARN" "VAPID subject not found in server/.env.production"
    fi
...
echo "Starting server directly to test VAPID keys..."
...
echo ""
echo "🏁 VAPID Keys Location Fix Complete!"
echo "==================================="

echo ""
echo "📋 VAPID Keys Location Fix Summary:"
echo "- ✅ PM2 processes stopped"
echo "- ✅ VAPID keys found in server directory"
echo "- ✅ VAPID keys copied from server/.env.production"
...
echo ""
echo "🎯 VAPID keys location issue fixed!";
