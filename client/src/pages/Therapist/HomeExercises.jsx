...
const HomeExercises = () => {
    ...
    () => therapistAPI.getPatients(user?.id),
    {
      ...
      onSuccess: (data) => {
        const transformedPatients =
          data?.data?.data?.patients?.map((patient) => ({
            id: patient.id,
  ...
  const { isRefreshing: isRefreshingProofs } = useRealtimeData(
    "therapistHomeExerciseProofs",
    refetchProofs,
  );

  // Create exercise mutation
  const createExerciseMutation = useMutation(therapistAPI.createHomeExercise, {
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      toast.error(
        `Failed to create exercise: ${error.response?.data?.error || error.message}`,
      );
    },
  });
  ...
  const updateExerciseMutation = useMutation(
    ({ id, exerciseData }) => therapistAPI.updateHomeExercise(id, exerciseData),
    {
      ...
      onError: (error) => {
        ...
        toast.error(
          `Failed to update exercise: ${error.response?.data?.error || error.message}`,
        );
      },
    },
  );
  ...
  const deleteExerciseMutation = useMutation(therapistAPI.deleteHomeExercise, {
    onSuccess: () => {
      queryClient.invalidateQueries("therapistHomeExercises");
    ...
    onError: (error) => {
      ...
      toast.error(
        `Failed to delete exercise: ${error.response?.data?.error || error.message}`,
      );
    },
  });
  ...
  const reviewProofMutation = useMutation(
    ({ proofId, reviewData }) =>
      therapistAPI.reviewHomeExerciseProof(proofId, reviewData),
    {
      onSuccess: () => {
        queryClient.invalidateQueries("therapistHomeExerciseProofs");
        queryClient.invalidateQueries("therapistHomeExercises");
      ...
      onError: (error) => {
        ...
        toast.error(
          `Failed to review proof: ${error.response?.data?.error || error.message}`,
        );
      },
    },
  );
  ...
  const filteredExercises = exercises.filter((exercise) => {
    const matchesSearch =
      ...
      exercise.patientFirstName
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      exercise.patientLastName.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
      statusFilter === "all" || exercise.status === statusFilter;
    const matchesPatient =
      patientFilter === "all" ||
      exercise.patientId.toString() === patientFilter;
  ...
  const handleViewProofs = (exercise) => {
    const exerciseProofs = proofs.filter(
      (proof) => proof.exerciseId === exercise.id,
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-100">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="divide-y divide-gray-200">
              {filteredExercises.map((exercise) => (
                <div
                  key={exercise.id}
                  className="p-3 sm:p-6 hover:bg-gray-50 transition-colors duration-200"
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 mb-2">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                          ...
                          <span
                            className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium self-start ${getStatusColor(exercise.status)}`}
                          >
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                        ...
                        <span
                          className={`inline-flex items-center px-2 py-1 sm:px-2.5 sm:py-1 rounded-full text-xs font-medium ${getDifficultyColor(exercise.difficulty)}`}
                        >
                          {exercise.difficulty}
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {selectedExercise
                        ...
                        : "Create a new home exercise for your patient"}
                    </p>
                  </div>
                </div>
                <button
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(proof.status)}`}
                          >
                          ...
                          <div className="mt-2">
                            <div className="flex items-center space-x-2 text-sm text-gray-600 mb-2">
                              ...
                              <span className="text-xs text-gray-500">
                                (
                                {proof.fileSize
                                  ? Math.round(proof.fileSize / 1024)
                                  : 0}{" "}
                                KB)
                              </span>
                            </div>
                              ...
                              proof.fileUrl && (
                                <div className="mt-2">
                                  <video
                                    ...
                                    onLoadStart={() => {
                                      const videoUrl = getProofImageUrl(
                                        proof.fileUrl,
                                      );
                                    ...
                                    onError={(e) => {
                                      ...
                                      const isMovFile = proof.fileName
                                        ?.toLowerCase()
                                        .endsWith(".mov");
                                      ...
                                      console.error("❌ Video load error:", {
                                        ...
                                        mimeType: proof.mimeType,
                                        fileName: proof.fileName,
                                        constructedUrl: getProofImageUrl(
                                          proof.fileUrl,
                                        ),
                                        isFormatError,
                                        isMovFile,
                                  ...
                                  {videoErrors[proof.id] && (
                                    <div className="text-sm text-red-500 mt-2 video-error-message">
                                      ...
                                      >
                                        ...
                                        <span>
                                          Download video to play in external
                                          player
                                        </span>
                                      </a>
                                    </div>
                      ...
                      {proof.status === "submitted" && (
                        <div className="flex flex-col gap-2 sm:mt-0 flex-shrink-0 sm:w-40">
                          <button
                            onClick={() =>
                              handleReviewProof(proof.id, "approved", "")
                            }
      ...
      <FullScreenImageViewer
        ...
        onClose={() =>
          setFullScreenImage({ isOpen: false, url: "", fileName: "" })
        }
