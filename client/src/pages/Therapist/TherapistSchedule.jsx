...
const TherapistSchedule = () => {
        ...
        try {
          ...
          const matchingPatient = patients.find((p) => {
            ...
            return (
              ...
              location.state.patientName.toLowerCase()
            );
          });
  ...
  const [selectedPatientForSchedule, setSelectedPatientForSchedule] =
    useState(null);
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery("therapistSchedule", therapistAPI.getSchedule, {
    onError: (error) => {
  ...
  const allAppointments = useMemo(() => {
    ...
    const currentTherapistId =
      allEvents.length > 0 ? allEvents[0].therapistId : null;
    ...
    return filteredEvents.map((appointment) => {
      const appointmentDate = new Date(
        appointment.appointmentDate || appointment.sessionDate || new Date(),
      );
      const appointmentTime =
        appointment.startTime || appointment.time || "09:00";
      ...
      const processedAppointment = {
        ...
        dateTime: dateTime,
        endTime: new Date(
          dateTime.getTime() + (appointment.duration || 60) * 60000,
        ),
  ...
  const filteredAppointments = useMemo(() => {
    ...
    filtered = filtered.filter(
      (appointment) => appointment.status !== "therapy",
    );
  ...
  const handleUpdateAppointment = async () => {
    ...
    if (
      ...
      toast.error(
        "Please fill in all required fields (Date, Start Time, and Type)",
      );
      return;
    }
    ...
    try {
      const statusToSave =
        editFormData.status === "pending" ? "scheduled" : editFormData.status;
  ...
  const handleApproveAppointment = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error || "Failed to approve appointment",
      );
    }
  };
              ...
              <div className="divide-y divide-gray-200 min-w-[400px] sm:min-w-[800px]">
                {filteredAppointments.length === 0 ? (
                  ...
                  currentAppointments.map((appointment) => {
                    ...
                    if (
                      appointmentType.includes("session") ||
                      appointmentType.includes("therapy") ||
                      ...
                      appointmentType.includes("assessment") ||
                      appointmentType.includes("evaluation")
                    ) {
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-12 gap-4 items-center">
                          ...
                          <div className="col-span-1 sm:col-span-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-medium border ${getTypeColor(appointment.type)}`}
                              >
          ...
          <SessionCreator
            ...
            patientName={
              selectedPatientForSchedule
                ? `${selectedPatientForSchedule.firstName || ""} ${selectedPatientForSchedule.lastName || ""}`.trim()
                : null
            }
            initialSessionType={
              location.state?.appointmentType === "assessment"
                ? "assessment"
                : null
            }
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-gray-900">
                          <Calendar className="h-4 w-4 text-gray-400" />
                          <span>
                            {selectedAppointment.dateTime.toLocaleDateString(
                              "en-US",
                              {
                                weekday: "long",
                                year: "numeric",
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium border ${getTypeColor(selectedAppointment.type)}`}
                          >
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(selectedAppointment.status)}`}
                          >
                          ...
                          <span className="text-gray-900">
                            {selectedAppointment.creatorRole === "admin"
                              ...
                              : selectedAppointment.creatorRole === "therapist"
                                ? "Therapist"
                                : selectedAppointment.creatorRole === "patient"
                                  ? "Patient"
                                  : "Unknown"}
                          </span>
                        </div>
                          ...
                          <div>
                            ...
                            <div className="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-lg flex items-center gap-2">
                              ...
                              <span className="text-sm">
                                This appointment requires admin approval
                              </span>
                            </div>
                          </div>
                        )}
                      ...
                      {selectedAppointment.creatorRole === "admin" && (
                        <div>
                          ...
                          <div className="bg-gray-50 border border-gray-200 text-gray-700 p-3 rounded-lg flex items-center gap-2">
                            <AlertTriangle className="h-4 w-4" />
                            <span className="text-sm">
                              Cannot edit admin-created appointments
                            </span>
                          </div>
                        </div>
                      )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Appointment approval is handled separately from the status
                      field.
                    </p>
                  </div>
                </div>
