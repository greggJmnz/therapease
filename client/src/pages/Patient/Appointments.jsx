...
const Appointments = () => {
  ...
  } = useQuery("patientAppointments", patientAPI.getAppointments, {
    onError: (error) => {
        ...
        if (data?.data?.therapistId) {
          setAssignedTherapist({
            ...
            name:
              ...
              "Your Therapist",
            specialization:
              data.data.therapistSpecialization ||
              data.data.therapist?.specialization ||
              "Occupational Therapy",
          });
        }
      },
  ...
  } = useQuery("patientTherapists", patientAPI.getTherapists, {
    onSuccess: (response) => {
      if (response?.data?.data && Array.isArray(response.data.data)) {
        setAvailableTherapists(response.data.data);
        // Set default to primary therapist if available
        const primaryTherapist = response.data.data.find(
          (t) => t.assignmentType === "primary",
        );
        if (primaryTherapist) {
  ...
  useEffect(() => {
    if (therapistsData?.data?.data && Array.isArray(therapistsData.data.data)) {
      ...
      // Set default to primary therapist if available
      const primaryTherapist = therapistsData.data.data.find(
        (t) => t.assignmentType === "primary",
      );
      if (primaryTherapist) {
        setSelectedTherapist(primaryTherapist.therapistId.toString());
  ...
  const resetForm = () => {
    ...
    if (Array.isArray(availableTherapists)) {
      const primaryTherapist = availableTherapists.find(
        (t) => t.assignmentType === "primary",
      );
      if (primaryTherapist) {
        setSelectedTherapist(primaryTherapist.therapistId.toString());
  ...
  const postponeAppointment = async (
    ...
    try {
      await patientAPI.postponeAppointment(
        appointmentId,
        newDate,
        newTime,
        reason,
                      ...
                      >
                        {availableTherapists.map((therapist) => (
                          ...
                          >
                            ...
                            {therapist.assignmentType === "primary"
                              ? " (Primary)"
                              : " (Secondary)"}
                          </option>
                        ))}
                        ...
                        <div className="grid grid-cols-9 gap-2 sm:gap-4 items-center">
                          ...
                          <div className="col-span-3">
                            <div className="flex items-center gap-1 sm:gap-2">
                              ...
                              <span
                                className={`px-1 py-0.5 sm:px-2 sm:py-1 rounded-full text-xs font-medium border ${getStatusColor(appointment.status)}`}
                              >
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-medium border whitespace-nowrap ${getStatusColor(selectedAppointment.status)}`}
                          >
