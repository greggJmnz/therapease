...
const DailyNotes = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery("patientDailyNotes", patientAPI.getDailyNotes, {
  ...
  const addCommentMutation = useMutation(
    async ({ noteId, comment }) => {
      const response = await fetch(
        buildApiUrl(`/api/patient/daily-notes/${noteId}/comments`),
        {
          method: "POST",
          headers: {
  ...
  const editCommentMutation = useMutation(
    ({ noteId, commentId, comment }) =>
      patientAPI.editNoteComment(noteId, commentId, comment),
    {
      onSuccess: () => {
        queryClient.invalidateQueries("patientDailyNotes");
  ...
  const notesArray = Array.isArray(notesData?.data?.data)
    ? notesData.data.data
    : [];
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-500">
          You need to be logged in to view your daily notes.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-500 mb-4">
          There was a problem loading your daily notes.
        </p>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div>
                ...
                <p className="mt-3 text-sm sm:text-base lg:text-lg text-gray-600">
                  View your therapy session notes and communicate with your
                  therapist
                </p>
              </div>
                                  ...
                                  <div className="flex items-center justify-between mt-3">
                                    <div className="flex items-center space-x-3">
                                      <button
                                        type="button"
                                        onClick={() =>
                                          handleAttachmentUpload(note.id)
                                        }
                                        className="inline-flex items-center px-3 py-1 text-xs font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors duration-200"
                                      >
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100">
              <div className="text-center py-20">
                ...
                <p className="text-lg text-gray-500 mb-8 max-w-md mx-auto">
                  Your therapist will add session notes here after each
                  appointment. Check back after your next session!
                </p>
              </div>
            </div>
          )}
