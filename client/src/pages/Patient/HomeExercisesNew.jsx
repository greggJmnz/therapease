...
const HomeExercisesNew = () => {
  ...
  const submitProofMutation = useMutation(
    ({ exerciseId, formData }) =>
      patientAPI.submitHomeExerciseProof(exerciseId, formData),
    {
      onSuccess: async () => {
  ...
  const updateStatusMutation = useMutation(
    ({ exerciseId, status }) =>
      patientAPI.updateExerciseStatus(exerciseId, status),
    {
      onSuccess: async () => {
        await queryClient.invalidateQueries([
          "patientHomeExercisesNew",
      ...
      onError: (error) => {
        ...
        toast.error(
          error.response?.data?.error || "Failed to update exercise status",
        );
      },
    },
  );
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 mb-2">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                            ...
                            <span
                              className={`inline-flex items-center px-2 sm:px-2.5 py-1 rounded-full text-xs font-medium self-start ${getStatusColor(exercise.status || "assigned")}`}
                            >
                              {(exercise.status || "assigned").replace(
                                "_",
                                " ",
                              )}
                            </span>
                        ...
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                          <div className="flex items-center text-xs sm:text-sm text-gray-600">
                            ...
                            <span className="truncate">
                              {exercise.therapistFirstName}{" "}
                              {exercise.therapistLastName}
                            </span>
                          </div>
                          <div className="flex items-center text-xs sm:text-sm text-gray-600">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mt-3 sm:mt-4">
                      ...
                      {exercise.status !== "in_progress" &&
                        exercise.status !== "completed" && (
                          <button
                            onClick={() =>
                              handleUpdateStatus(exercise.id, "in_progress")
                            }
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-2/3 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="space-y-4">
                {!Array.isArray(exerciseProofs) ||
                exerciseProofs.length === 0 ? (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <span
                              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getProofStatusColor(proof.status)}`}
                            >
                                  ...
                                  <div className="mt-2">
                                    ...
                                    {videoErrors[proof.id] && (
                                      <div className="text-sm text-red-500 mt-2 video-error-message">
                                        ...
                                        <a
                                          href={getProofImageUrl(
                                            proof.fileUrl || proof.filePath,
                                          )}
                                          download={
                                            proof.fileName || "video.mov"
                                          }
